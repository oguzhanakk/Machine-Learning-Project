...
class autogluon:
    ...
    def predict(predictor,test_data, include_history=False):
        
        predictions = predictor.predict(test_data, include_history=include_history)
        
        item_id = 'Brand'
        ytrue = test_data.loc[item_id]["Value"]
        ypred = predictions.loc[item_id]
        ypred.loc[ytrue.index[-1]] = [ytrue[-1]] * 10
        ypred = ypred.sort_index()
        
        
        ytrue_test = test_data.loc[item_id]["Value"]
